...
*******************************************************************************
	...
	integer i,j,k,l,m,n,o,Nx,My,Ot,KPsi,index,kerr,kosc,report, Iter,
     c	LL, length, TT, yy, psave,tic, ct, rcc, flag, IBL,ia,ib
	double precision Xmin, Xmax, Ymin, Ymax, xi, Umax, Re, C,
     ...
     c	Amp,pi,Lambda,freq,yhalf,t,c0,amewa,f,A,d7
...
*	Lower 
	...
	Psi(i,1)=(-c0*(0.0-.5*amewa*sqrt(amewa**2+1.0d0)-.5*Dsinh(amewa)
     c  +.5*x(i)*sqrt(x(i)**2+1)+.5*Dsinh(x(i))))*f
...
*WEIRD THINGS GOING ON HERE with top right corner of output!!!!!!!!!!!!!
		
		if ((i.ge.(ia-1)) .and. (i.le.(ib+1))) then
		
	Omega(i,1) = Omega(i,1)
     c  +(v(i+1,1)*sqrt(x(i+1)**2+1)-v(i-1,1)*sqrt(x(i-1)**2+1))
     c  /(2.0*dx)/d6
